...
(define (Exercise-2-57)
  ...
  (define (make-sum a1 a2) 
    (cond ((=number? a1 0) a2)
          ...
          (else (list '+ a1 a2))))
  ; *
  (define (multiplier p) (cadr p)) ; (* x y z) --> x
  (define (multiplicand p) ; (* x y z) --> (* y z)
    (let ((m1 (caddr p)) ; y
          (m2 (cdddr p))) ; z
      (if (null? m2)
        m1
...
(say "Exercise 2.61 & 2.62")
(define (Exercise-2-61-and-2-62)
  (define (element-of-set? x set)
    (cond ((null? set) #f)
          ((= x (car set)) #t)
          ((< x (car set)) #f)
  ...
  (define set1 (adjoin-set 2 (adjoin-set 4 (adjoin-set 1 '()))))
  (define set2 (adjoin-set 7 (adjoin-set 1 (adjoin-set 4 '()))))
  (say set1)
  (say set2))
